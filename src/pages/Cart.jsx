...
import { PizzaInCart, EmptyCart, Button } from '../components';
import { useSelector, useDispatch } from 'react-redux';
import { delAllCart, delItemCart, plusCart, minusCart } from '../redux/actions/cart';
...
function Cart() {
    ...
    const addedPizzas = Object.keys(items).map((elem) => {
        return items[elem][0];
    })
    ...
    const deletePizzas = (obj) => {
        if (window.confirm('Удалить пиццу?')) {
            dispatch(delItemCart(obj));
        }
    }

    const plusPizza = (obj) => {
        dispatch(plusCart(obj));
    }

    const minusPizza = (obj) => {
        if (totalCount === 1) {
            dispatch(delAllCart());
        } else {
            dispatch(minusCart(obj));
        }
    ...
    return (
        <div className="content">
            {Object.keys(items).length !== 0 ? <div className="container container--cart">
                <div className="cart">
                    ...
                    {addedPizzas.map((object, id) => (
                        object
                            ? <PizzaInCart key={`${id}_${object.name}`} name={object.name} type={object.type} size={object.size} url={object.imageUrl} price={object.price} id={object.id} deleteItems={deletePizzas} plusItemPizza={plusPizza} minusItemPizza={minusPizza} />
                            : ''
                    ))
                    }
                    <div className="cart__bottom">
                        ...
                        <div className="cart__bottom-buttons">
                            ...
                            <Button className="pay-btn" onClick={() => console.log('Заказ:', items)}>
